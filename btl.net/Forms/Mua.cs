...
namespace btl.net.Forms
{
    public partial class Mua : Form
    {
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bạn phải nhập Tên mùa", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtten.Focus();
                return;
            }
            sql = "SELECT Mamua FROM tblmua WHERE Mamua=N'" + txtma.Text.Trim() + "'";
            if (Class.Functions.CheckKey(sql))
            {
                MessageBox.Show("Mã mùa này đã có, bạn phải nhập mã khác", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                txtma.Focus();
                txtma.Text = "";
                return;
            }
            sql = "INSERT INTO tblmua(Mamua, Tenmua) VALUES(N'" + txtma.Text.Trim() + "',N'" + txtten.Text.Trim() + "')";
        ...
        private void btnsua_Click(object sender, EventArgs e)
        {
            ...
            sql = "UPDATE tblmua SET Tenmua=N'" + txtten.Text.ToString() + "' WHERE Mamua=N'" + txtma.Text + "'";
            Class.Functions.RunSql(sql);
            Load_DataGridView();
            ResetValues();
            btnboqua.Enabled = false;
        ...
        private void btnxoa_Click(object sender, EventArgs e)
        {
            ...
            {
                sql = "DELETE FROM tblmua WHERE Mamua=N'" + txtma.Text + "'";
        ...
        private void btntimkiem_Click(object sender, EventArgs e)
        {
            ...
            sql = "SELECT * FROM tblmua WHERE 1=1";
            if (txtma.Text != "")
                sql += " AND Mamua Like N'%" + txtma.Text + "%'";
            if (txtten.Text != "")
                sql += " AND Tenmua Like N'%" + txtten.Text + "%'";
            tblchatlieu = Class.Functions.GetDataToTable(sql);
            if (tblchatlieu.Rows.Count == 0)
        ...
        private void DataGridView_Click_1(object sender, EventArgs e)
        {
            ...
            txtma.Text = DataGridView.CurrentRow.Cells["Mamua"].Value.ToString();
            txtten.Text = DataGridView.CurrentRow.Cells["Tenmua"].Value.ToString();
            btnsua.Enabled = true;
            btnxoa.Enabled = true;
            btnboqua.Enabled = true;
        }
